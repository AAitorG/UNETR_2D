...
def YNETR_2D(
    ...
    for layer in range(total_upscale_factor):
        x = Conv2D(num_filters * (2**(layer)), (3, 3), activation=None, kernel_initializer=kernel_init, padding='same') (x)
        x = BatchNormalization() (x) if batch_norm else x
        x = Activation(activation) (x)
        if dropout[layer] > 0.0:
            x = Dropout(dropout[layer]) (x)
        x = Conv2D(num_filters * (2**(layer)), (3, 3), activation=None, kernel_initializer=kernel_init, padding='same') (x)
        x = BatchNormalization() (x) if batch_norm else x
        x = Activation(activation) (x)
    ...
    # U-Net BOTTLENECK
    x = Conv2D(num_filters * (2**(total_upscale_factor)), (3, 3), activation=None, kernel_initializer=kernel_init, padding='same')(x)
    x = BatchNormalization() (x) if batch_norm else x
    x = Activation(activation) (x)
    ...
    x = Conv2D(num_filters * (2**(total_upscale_factor)), (3, 3), activation=None, kernel_initializer=kernel_init, padding='same') (x)
    x = BatchNormalization() (x) if batch_norm else x
    unet_bn = Activation(activation) (x)

    ### UNETR convolutional Part (bottom_up)

    # bottleneck
    b = Reshape([   input_shape[0]//patch_size,
                    input_shape[1]//patch_size,
                    projection_dim])(encoded_patches) # use the encoder output (easier to try different things)
    x = concatenate([b, unet_bn], axis = 3, name='BN_concat')
    if dropout[total_upscale_factor] > 0.0:
        x = SpatialDropout2D(dropout[total_upscale_factor], name='BN_Dropout2d')(x) 
    x = up_green_block(x, num_filters * (2**(total_upscale_factor-1)), name='BN_TConv' )

    for layer in reversed(range(1, total_upscale_factor)):
        # skips (with blue blocks)
        z = Reshape([   input_shape[0]//patch_size,
                        input_shape[1]//patch_size,
                        projection_dim])( 
        ...
        # decoder
        x = concatenate([z, x, unet_skips[layer]], axis=3)
        if dropout[layer] > 0.0:
            x = SpatialDropout2D(dropout[layer])(x)
    ...
    x = concatenate([long_skip, x, unet_skips[0]], axis=3)
    if dropout[0] > 0.0:
        x = SpatialDropout2D(dropout[0])(x)
